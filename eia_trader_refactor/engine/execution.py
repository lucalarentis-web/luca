...
class PaperBroker:
    ...
    def enter(self, side: str, qty: int, q: Quote) -> float:
        ...
        self.pos = Position(side=side, qty=int(qty), entry_price=fill, entry_time=datetime.now(), best_price=fill)
        return float(fill)
    ...
    def exit(self, q: Quote) -> float:
        ...
        pnl = (fill - self.pos.entry_price) * self.pos.qty if self.pos.side == "LONG" else (self.pos.entry_price - fill) * self.pos.qty
